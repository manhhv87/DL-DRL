...
class StateTAOP(NamedTuple):
    ...
    def visited(self):
        if self.visited_.dtype == torch.uint8:
            ...
            return (
                self.visited_[:, None, :]
                .expand(self.visited_.size(0), 1, -1)
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        return StateTAOP(
            ...
            veh=torch.arange(StateTAOP.veh_num, dtype=torch.int64, device=loc.device)[
                :, None
            ],
            # prev_a is current node
            prev_a=torch.zeros(
                batch_size, StateTAOP.veh_num, dtype=torch.long, device=loc.device
            ),
            ...
            cur_coord=input["depot"][:, None, :].expand(
                batch_size, StateTAOP.veh_num, -1
            ),
            # Add step dimension
            i=torch.zeros(
    ...
    def update(self, selected, veh):  # [batch_size, num_veh]
        ...
        cur_coord = self.coords.gather(  # [batch_size, num_veh, 2]
            ...
            selected[:, :, None].expand(
                selected.size(0), self.veh_num, self.coords.size(-1)
            ),
        )

        if self.visited_.dtype == torch.uint8:
            visited_ = self.visited_.scatter(
                -1,
                prev_a[torch.arange(batch_size), veh][:, None, None].expand_as(
                    self.visited_[:, :, 0:1]
                ),
                1,
            )
        ...
        return self._replace(
            prev_a=prev_a, visited_=visited_, cur_coord=cur_coord, i=self.i + 1
        )
    ...
    def get_mask(self, veh):
        ...
        exceeds_cap = self.demand[self.ids, 1:] + (
            self.used_capacity[torch.arange(batch_size), veh].unsqueeze(-1)
        )[..., None].expand_as(self.demand[self.ids, 1:]) > (
            (
                torch.tensor(self.VEHICLE_CAPACITY)[None, :]
                .expand(batch_size, len(self.VEHICLE_CAPACITY))
                .cuda()
            )[self.ids, veh][0].unsqueeze(-1)
        )[
            ..., None
        ].expand_as(
        ...
        mask_loc = (
            visited_loc.to(exceeds_cap.dtype) | exceeds_cap
        )  # [batch_size, 1, n_loc]
