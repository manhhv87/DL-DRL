...
class AttentionModel(nn.Module):
    ...
    def forward(
        ...
        return_pi=False,
        Lval_dataset=False,
        no_attention_layer=False,
        return_allocate=False,
    ):
        """
        ...
        log_p_veh, pi, veh_list, tour_1, tour_2, tour_3, tour_4 = self._inner(
            input, embeddings
        )  # _log_p: [batch_size, graph_size+1, graph_size+1], pi:[batch_size, graph_size+1]

        Lmask_dataset1, Lmask_dataset2, Lmask_dataset3, Lmask_dataset4 = (
            self.problem.LmaskDataset(input, tour_1, tour_2, tour_3, tour_4)
        )

        if Lval_dataset:
            return Lmask_dataset1, Lmask_dataset2, Lmask_dataset3, Lmask_dataset4

        cost, mask, Ttour_1, Ttour_2, Ttour_3, Ttour_4 = self.problem.get_costs(
            Lmodel,
            L_train,
            Lmask_dataset1,
            Lmask_dataset2,
    ...
    def select_veh(
        ...
        if sequences:
            ...
            tour_con_1 = torch.gather(
                ...
                batch_size, tour_1.size(-1), embed_dim
        ...
        context = torch.cat((next_nodeembedding, tour_context), -1).view(
    ...
    def _inner(self, input, embeddings):
        ...
        return (
            ...
            veh_list,
            torch.stack(tour_1, -1),
            torch.stack(tour_2, -1),
            torch.stack(tour_3, -1),
            torch.stack(tour_4, -1),
        )
    ...
    def sample_many(self, Lmodel, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            lambda input, tour_1, tour_2, tour_3, tour_4: self.problem.LmaskDataset(
                input, tour_1, tour_2, tour_3, tour_4
            ),
            lambda Lmodel, Ltrain, dataset_1, dataset_2, dataset_3, dataset_4: self.problem.get_costs(
                Lmodel, Ltrain, dataset_1, dataset_2, dataset_3, dataset_4
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        (
            ...
            glimpse_val_fixed,
            logit_key_fixed,
        ) = self.project_node_embeddings(embeddings[:, None, :, :]).chunk(
            3, dim=-1
        ...
        fixed_attention_node_data = (  # make multihead
            ...
            ),  # (n_heads, batch_size, num_steps, graph_size+1, embed_dim/n_heads)
            self._make_heads(
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, veh):
        ...
        if self.mask_inner:  # True
            ...
            compatibility[mask[None, :, :, None, :].expand_as(compatibility)] = (
                -math.inf
            )  # nask visited nodes and nodes cannot be visited
