...
def get_best(sequences, cost, veh_lists, ids=None, batch_size=None):
    ...
    return (
        [sequences[i] if i >= 0 else None for i in result],
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    if opts.multiprocessing:
        ...
        results = _eval_dataset(
            model, Lmodel, dataset, width, softmax_temp, opts, device
        )

    # This is parallelism, even if we use multiprocessing (we report as if we did not use multiprocessing, e.g. 1 GPU)
    parallelism = opts.eval_batch_size

    costs, tours, veh_lists, durations = zip(
        *results
        ...
        with torch.no_grad():
            if opts.decode_strategy in ("sample", "greedy"):
                ...
                sequences, costs, veh_lists = model.sample_many(
                    Lmodel, batch, batch_rep=batch_rep, iter_rep=iter_rep
                )
                print("cost", costs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=r"F:\论文-Large-scale task scheduling of multi-UAV A deep reinforcement learning approach\TAOP-6-altTrain\fleet_v6\outputs\taop_80\run_20211226T012743\epoch-99.pt",
    )
    parser.add_argument(
        "--Lmodel",
        type=str,
        default=r"F:\论文-Large-scale task scheduling of multi-UAV A deep reinforcement learning approach\TAOP-6-altTrain\fleet_v6\outputs\op_80\run_20211226T012743\epoch-1098.pt",
    )
    ...
    parser.add_argument(
        "--max_calc_batch_size", type=int, default=10000000, help="Size for subbatches"
    )
    parser.add_argument(
    ...
    opts.datasets = [
        r"F:\论文-Large-scale task scheduling of multi-UAV A deep reinforcement learning approach\TAOP-6-altTrain\fleet_v6\data\taop_500_seed1234.pkl"
    ]
