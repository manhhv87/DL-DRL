...
def solve_euclidian_op(
    depot, loc, prize, max_length, threads=0, timeout=None, gap=None
):
    """
    ...
    prize_dict = {
        i + 1: -p for i, p in enumerate(prize)  # We need to maximize so negate
    }
    ...
    m.addConstr(
        quicksum(var * dist[i, j] for (i, j), var in vars.items() if j < i)
        <= max_length
    )
