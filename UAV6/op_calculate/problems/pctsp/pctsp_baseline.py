...
def get_pctsp_executable():
    ...
    if not os.path.isfile(execfile):
        ...
        print("Done!")
    assert os.path.isfile(execfile), "{} does not exist! Compilation failed?".format(
        execfile
    )
...
def solve_pctsp_log(
    ...
    try:
        ...
        if not os.path.isfile(output_filename):
            write_pctsp(
                problem_filename, depot, loc, penalty, deterministic_prize, name=name
            )
        ...
        return (
            calc_pctsp_cost(depot, loc, penalty, deterministic_prize, tour),
            tour.tolist(),
            duration,
        )
    ...
    try:
        ...
        if not os.path.isfile(output_filename):
            ...
            while len(final_tour) < len(stochastic_prize):
                ...
                if (
                    len(final_tour) > 0
                ):  # in the first iteration we are at depot and distance matrix is ok
                    dist[0, :] = dist[final_tour[-1], :]

                remaining_deterministic_prize = deterministic_prize[~mask[1:]]
                write_pctsp_dist(
                    problem_filename,
                    dist[np.ix_(~mask, ~mask)],
                    penalty[~mask[1:]],
                ...
                min_prize_int = max(
                    0,
                    min(
                        float_to_scaled_int(1.0 - total_collected_prize),
                        sum(
                            [
                ...
                if len(tour_node_ids) == 0:
                    ...
                    assert (
                        total_collected_prize > 1 - 1e-5
                    ), "Collected prize should be one"
                    break
            ...
            final_cost = calc_pctsp_cost(
                depot, loc, penalty, stochastic_prize, final_tour
            )
...
def solve_salesman(
    directory, name, depot, loc, penalty, deterministic_prize, stochastic_prize, runs=10
):
    ...
    try:
        ...
        if not os.path.isfile(output_filename):
            ...
            random.seed(1234)
            pctsp = Pctsp()
            pctsp.load(problem_filename, float_to_scaled_int(1.0))
    ...
    try:
        ...
        if os.path.isfile(problem_filename) and not disable_cache:
            ...
            cost, tour = solve_euclidian_pctsp_gurobi(
                ...
                penalty,
                deterministic_prize,
                min(sum(deterministic_prize), 1.0),
                threads=1,
                timeout=timeout,
                gap=gap,
            )
...
def calc_pctsp_cost(depot, loc, penalty, prize, tour):
    ...
    assert calc_pctsp_total(prize, tour) >= 1 - 1e-5 or len(tour) == len(
        prize
    ...
    return (
        calc_pctsp_length(depot, loc, tour)
        + np.sum(penalty)
        - calc_pctsp_total(penalty, tour)
    )
...
if __name__ == "__main__":
    ...
    for dataset_path in opts.datasets:
        ...
        if (
            method in ("pctsp", "salesman", "gurobi", "gurobigap", "gurobit", "ortools")
            or method[:10] == "stochpctsp"
        ):
            ...
            if method[:6] == "gurobi":
                ...
                def run_func(args):
                    return solve_stochastic_pctsp_log(
                        executable, *args, runs=runs, append=append
                    )
